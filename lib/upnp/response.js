...
function parseEvents(buf) {
  const xml = decode(buf.toString(), {level: 'xml'});
  const parser = new XMLParser({htmlEntities: true, attributeNamePrefix: '__', ignoreAttributes: false});
  const body = parser.parse(xml);

  const props = body['e:propertyset']['e:property'];

  if (props.LastChange) {
    const InstanceID = props.LastChange.Event.InstanceID;
